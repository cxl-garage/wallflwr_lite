...
def main(attempts=1):
    try:
        ...
        insights = pd.DataFrame(columns=['committed_sql', 'committed_images', 'committed_lora', 'insight_id', 'alg_id', 'time_stamp',
    ...
    # bytearray([ 0x26, 0x02, 0x1E, 0x47 ])
    devaddr = bytearray.fromhex(str(device_info['lora_devaddr'][0]))
    ...
    while k < len(x):
        if x['group_confidence'][k] != 0:
            try:
                ...
                local_insight_id_1 = int(
                    math.floor(int(x['insight_id'][k])/256))
                local_insight_id_2 = int(x['insight_id'][k]) % 256
                data = [alg_id_1, alg_id_2, class_id, confidence,
                        local_insight_id_1, local_insight_id_2]
                m = 0
                while m < attempts:
                    ...
                    m = m + 1
                time.sleep(0.5)
                insights.loc[insights['group_id'] ==
                             x['group_id'][k], 'committed_lora'] = True
                insights.to_csv('../data/device_insights.csv')
                logger.info('LoRa Packet Sent! (Algorithm: {}, Class ID: {}, Group Confidence {}, Insight ID: {})'.format(
                    data[0]*256+data[1], data[2], data[3], data[4]*256+data[5]))
            except Exception as e:
                logger.error('Issue sending insight {}'.format(
                    x['insight_id'][k]))
        k = k + 1
