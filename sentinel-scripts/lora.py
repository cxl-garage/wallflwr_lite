...
def main(attempts=1):
	...
	device_info = pd.read_csv('../_device_info.csv')
	devaddr = device_info['lora_devaddr'][0]#bytearray([ 0x26, 0x02, 0x1E, 0x47 ])

	# TTN Network key
	#nwkey = bytearray(device_info['lora_nwkey'][0],'utf-16')
	nwkey = device_info['lora_nwkey'][0]#bytearray([ 0x40, 0x3A, 0xCC, 0x3B, 0xB3, 0xE2, 0x60, 0xF6, 0xC4, 0x6A, 0xC8, 0x22, 0x7C, 0xC5, 0xFD, 0xC3 ])
	app   = device_info['lora_appkey'][0]

	logger.info('LoRa Info: Device Address: {}, Network Key: {}, Application Key: {}'.format(devaddr,nwkey,app)
	# TTN Application key
